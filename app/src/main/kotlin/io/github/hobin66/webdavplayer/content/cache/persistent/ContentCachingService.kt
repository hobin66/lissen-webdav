...
class ContentCachingService : LifecycleService() {
  ...
  private val executionStatuses = mutableMapOf<String, Pair<DetailedItem, CacheState>>()
  private val executingCaching = mutableMapOf<String, Job>()
  ...
  override fun onStartCommand(
    ...
    when (action) {
      CACHE_ITEM_ACTION -> cacheItem(intent).also { it?.let { (item, job) -> executingCaching[contentCachingKey(item)] = job } }
      STOP_CACHING_ACTION -> stopCaching(intent)
      STOP_ALL_CACHING_ACTION -> stopAllCaching()
    }
  ...
  private fun stopCaching(intent: Intent) {
    ...
    val cachingKey = contentCachingKey(cachingItem)

    val executingJob = executingCaching[cachingKey] ?: return

    lifecycleScope.launch {
      executingJob.cancel()

      cacheProgressBus.emit(cachingItem, CacheState(status = CacheStatus.Idle))
      executionStatuses.remove(cachingKey)
      executingCaching.remove(cachingKey)
      finish()
    }
  }

  private fun stopAllCaching() {
    lifecycleScope.launch {
      val runningJobs = executingCaching.toMap()
      val runningStatuses = executionStatuses.toMap()
      executingCaching.clear()
      executionStatuses.clear()

      runningJobs.forEach { (key, job) ->
        val item = runningStatuses[key]?.first ?: return@forEach
        job.cancel()
        cacheProgressBus.emit(
          item = item,
          progress = CacheState(status = CacheStatus.Idle),
        )
      }
  ...
  private fun cacheItem(intent: Intent): Pair<DetailedItem, Job>? {
    ...
    val item = task.item
    val cachingKey = contentCachingKey(item)

    executingCaching[cachingKey]?.cancel()
      ...
      lifecycleScope.launch {
        ...
        executor
          ...
          .onCompletion {
            ...
            executionStatuses[cachingKey] = item to progress
  ...
  private fun inProgress(): Boolean = executionStatuses.values.any { (_, state) -> state.status == CacheStatus.Caching }

  private fun hasErrors(): Boolean = executionStatuses.values.any { (_, state) -> state.status == CacheStatus.Error }
  ...
  companion object {
    ...
    const val STOP_ALL_CACHING_ACTION = "STOP_ALL_CACHING_ACTION"
...
internal fun contentCachingKey(item: DetailedItem): String = item.id
