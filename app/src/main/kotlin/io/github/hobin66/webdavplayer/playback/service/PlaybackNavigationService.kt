...
class PlaybackNavigationService
  ...
  constructor(
    ...
    private fun isTrackAvailable(mediaId: String): Boolean {
      ...
      return isChapterMediaItemAvailable(mediaItem, mediaId)
    }
  }

internal fun isChapterMediaItemAvailable(
  book: DetailedItem,
  mediaId: String,
): Boolean =
  WebdavPlayerMediaSourceFactory.MediaId
    .fromString(mediaId)
    ?.takeIf { it.bookId == book.id }
    ?.chapterId
    ?.let { chapterIndex -> book.chapters.getOrNull(chapterIndex)?.available == true }
    ?: false
