...
import io.github.hobin66.webdavplayer.ui.icons.TimerPlay
  ...
  Surface(
    ...
    NavigationBar(
      ...
      NavigationBarItem(
        ...
        icon = {
          Icon(
            ...
            contentDescription = stringResource(R.string.player_screen_skip_navigation),
            modifier = Modifier.size(iconSize),
          )
        },
        label = {
      ...
      NavigationBarItem(
        ...
        label = {
          when (timerOption) {
            is DurationTimerOption, CurrentItemTimerOption -> {
              Text(
                text =
                  timerRemaining
                    ?.toInt()
      ...
      if (timerExpanded) {
        TimerComposable(
          ...
          preferredStopMode = preferredSleepTimerStopMode,
          onOptionSelected = { playerViewModel.setTimer(it) },
          onPreferredStopModeSelected = { playerViewModel.setPreferredSleepTimerStopMode(it) },
          onDismissRequest = { timerExpanded = false },
        )
      }

      if (skipExpanded) {
        SkipIntroOutroComposable(
          book = book,
          onSave = { introSeconds, outroSeconds ->
            playerViewModel.saveBookSkipSettings(book.id, introSeconds, outroSeconds)
          },
