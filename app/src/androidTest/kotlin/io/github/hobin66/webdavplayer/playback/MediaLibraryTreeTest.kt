...
class MediaLibraryTreeTest {
  ...
  private lateinit var preferences: WebdavPlayerPreferences
  private lateinit var localCacheRepository: LocalCacheRepository
  private lateinit var webdavMediaProvider: WebdavMediaProvider
  ...
  fun setUp() {
    ...
    webdavMediaProvider = mockk(relaxed = true)

    every { preferences.getPlayingItem() } returns null
    every { preferences.getPreferredLibrary() } returns null

    coEvery { webdavMediaProvider.fetchLibraries() } returns
      OperationResult.Error(OperationError.InternalError)
    coEvery { webdavMediaProvider.fetchRecentListenedBooks(any()) } returns
      OperationResult.Error(OperationError.InternalError)
    coEvery { webdavMediaProvider.fetchBooks(any(), any(), any()) } returns
      OperationResult.Error(OperationError.InternalError)
    coEvery { localCacheRepository.fetchDetailedItems() } returns
      OperationResult.Error(OperationError.InternalError)

    tree = MediaLibraryTree(context, preferences, localCacheRepository, webdavMediaProvider)
  }
  ...
  fun getChildren_recent_returnsEmptyWhenProviderFails() =
    runBlocking {
      ...
      coEvery { webdavMediaProvider.fetchRecentListenedBooks("lib-1") } returns
  ...
  fun getChildren_nestedLibrary_returnsBooksFromLibrary() =
    runBlocking {
      ...
      coEvery { webdavMediaProvider.fetchBooks("lib-1", any(), any()) } returns
        OperationResult.Success(
          PagedItems(
            items =
              listOf(
  ...
  fun getItem_bookPath_returnsBookItem() =
    runBlocking {
      coEvery { webdavMediaProvider.fetchBook("book-1") } returns
  ...
  fun getItem_bookPath_returnsErrorWhenBookNotFound() =
    runBlocking {
      coEvery { webdavMediaProvider.fetchBook("missing-id") } returns
  ...
  fun searchBooks_returnsEmptyWhenProviderFails() =
    runBlocking {
      every { preferences.getPreferredLibrary() } returns makeLibrary("lib-1")
      coEvery { webdavMediaProvider.searchBooks("lib-1", "audiobook", any()) } returns
        OperationResult.Error(OperationError.NetworkError)
      val results = tree.searchBooks("audiobook").get()
      assertTrue(results.isEmpty())
    }
  ...
  fun searchBooks_returnsMatchingBooks() =
    runBlocking {
      every { preferences.getPreferredLibrary() } returns makeLibrary("lib-1")
      coEvery { webdavMediaProvider.searchBooks("lib-1", "dune", any()) } returns
        OperationResult.Success(
          listOf(
